...
local vars, i;
    ...
    return [seq(Groebner:-MultiplicationMatrix(vars[i], ns, rv, G, tord), i=1..nops(vars))];
end proc:
 ...
 local D, g, d, i, base, b, e, powers, s_list, f, local_g, krylov_vectors, result_mat, j, SFg,k; 
 D := RowDimension(A);
 ...
 if evalb(degree(g, op(indets(g))) = D_new) then return g, Atran, ns_new, G_new;
 ...
 c := [1, seq(i^j, j = 1 .. n-1)]; t := add(vars[j]*c[j], j = 1 .. n); M_t := add(Mlist[j]*c[j], j = 1 .. n);
 g1, Atran1 := minimalpoly(M_t); if evalb(degree(g1, op(indets(g1))) = D_new) then return g1, Atran1, ns_new,G_new; end if; end do; 
 ...
 local i, j, k, D_0, S, S1, n, P, L, U, V, X, Y, T, t, P1, m, l, G, Ns, ns, rv, Atran, g, vars, g1, ns_new,G_new,lt,lc,rest,other_vars,linear_polys,val_table,v,eqs,lm,pos,V1;
 G := Groebner:-Basis(F, tord); ns, rv := Groebner:-NormalSet(G, tord);
 ns := ListTools:-Reverse(ns); rv := table([seq(ns[i] = i, i = 1 .. nops(ns))]);
 vars := PolynomialIdeals:-IdealInfo[Variables](PolynomialIdeals:-PolynomialIdeal(G));
 n := nops(vars); 
 g, Atran, ns_new, G_new := IdealDeflation(n, ns, rv, G, tord);
 ...
 if n = D_0 -l-1 then 
        g1 := [seq(X[D_0 - m+i], i=1..m-1)];
    elif n > D_0 -l-1 then
        linear_polys := select(p -> degree(p) = 1, G_new);
        if nops(linear_polys) = 0 then
            error "no linear polynomial found，but n > D_0 -l-1";
        end if;
        lt := LeadingTerm(linear_polys[1], tord);
        lm := lt[2]; lc := lt[1];
        rest := expand(lc*lm - linear_polys[1])/lc;
        other_vars := indets(rest);
        val_table := table();
        for v in other_vars do 
            pos := ListTools:-Search(v, [seq(Ns[i], i=1..D_0 )]);
            if pos = 0 then
                error "Variable %1 is not in the regular set", v;
            end if;
            val_table[v] := X[pos,1];
        end do;
        eqs := [seq(v = val_table[v], v in other_vars)]; 
        V1 := expand(eval(rest, eqs));
        g1 := [V1, seq(X[D_0 -m+i], i=1..m-1)];
    else
        error "Unhandle case: n < D_0 -l-1";
    end if;
 return g1, g; 
 end proc:
